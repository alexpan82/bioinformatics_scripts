...
#filedir <- '/users/PAS0472/osu8725/tech_manuscript/transcriptome_figures/VAC05-C24/'
#title <- "VAC05 C24"
...
parseFiles <- function() {
	...
	data <- import("lcRNA.txt", "lc1", data)
	data <- import("lcRNA_2.txt", "lc2", data)
	data <- import("lcRNA_3.txt", "lc3", data)
...
data <- parseFiles(filedir)

#create label vector
axis_labels <- colnames(data)
axis_labels <- gsub("upm", "10X 5' (UPM,", axis_labels)
axis_labels <- gsub("sc", "scRNA (TPM,", axis_labels)
axis_labels <- gsub("lc1", "300 Cells (1) (TPM,", axis_labels)
axis_labels <- gsub("lc2", "300 Cells (2) (TPM,", axis_labels)
axis_labels <- gsub("lc3", "300 Cells (3) (TPM,", axis_labels)
axis_labels <- gsub("ng", "1ng (TPM,", axis_labels)
...
png(paste(title,".multirep.heatmaps.png",sep=""), width = 12, height=12, units = "in", res = 600)
