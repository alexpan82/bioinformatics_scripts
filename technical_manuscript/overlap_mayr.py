...
with pd.ExcelWriter("Table for " + Patient +"_spliced_withoutUTRs.xlsx") as writer:  
    BL3.to_excel(writer, sheet_name="_".join(BL3f.split('_')[2:4]))
    Y53.to_excel(writer, sheet_name="_".join(Y53f.split('_')[2:4]))
    BL5.to_excel(writer, sheet_name="_".join(BL5f.split('_')[2:4]))
    Y55.to_excel(writer, sheet_name="_".join(Y55f.split('_')[2:4]))
