...
def readMixcr(txtlist, uChain):
	...
	returndic_counts = defaultdict(lambda: defaultdict(lambda: 0))
	for txt in filelist:
		with open(txt, 'r') as f:
			...
			for cdr3 in returndic[name].keys():
				returndic_counts[name][cdr3] = returndic[name][cdr3]
				returndic[name][cdr3] = returndic[name][cdr3]/totalcount

	return returndic, returndic_counts
...
def plotBar(mixcrlist, plotcdr3counts, cdr3colors, axisBreak, uChain):
	...
	for k, f in enumerate(mixcrlist):
		...
		if 'Others' in plotcdr3counts[f].keys():
			...
			bar[0] = ax.bar(np.arange(len(mixcrlist)), data, width=0.7, linewidth=0.1, 
				color='0.85', bottom=nextbot)
				
			bar2[0] = ax2.bar(np.arange(len(mixcrlist)), data, width=0.7, linewidth=0.1, 
				color='0.85', bottom=nextbot)
...
def main(args):
	...
	cdr3counts, cdr3counts_scalars = readMixcr(mixcrlist, args.userChain)
	...
	for f in cdr3counts.keys():
		print(f, 'Number of clones: %s' % len(cdr3counts[f].keys()))
		others_sum_file = 0
		for index, cdr3 in enumerate(sorted(cdr3counts[f].keys(), key=lambda u: cdr3counts[f][u], reverse=True)):
			...
			if index > 9:
				...
				others_sum_file += cdr3counts_scalars[f][cdr3]
			else:
				print(cdr3, cdr3counts[f][cdr3]*100., cdr3counts_scalars[f][cdr3])
				plotcdr3counts[f][cdr3] = cdr3counts[f][cdr3]
		if plotcdr3counts[f]['Others'] > 0:
			print('Others', plotcdr3counts[f]['Others']*100., others_sum_file)
	...
	# Plot stacked bars
	# Also make legend
	plotBar(mixcrlist, plotcdr3counts, cdr3colors, axisBreak, args.userChain)
