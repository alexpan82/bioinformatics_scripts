...
#for FASTQ in $(cat test.txt);do
#for FASTQ in Nephew-Miller-1A-RRBS_S3_R1_001_trimmed_PART-001.fq.gz;do
...
echo '
#PBS -N job.'$FQNAME'.'$GENOME'.bismark-bt2_PE.job
...
#~osu6369/tools/bismark-current/bismark --bowtie2 --path_to_bowtie ~osu6369/tools/bowtie2-current/ --multicore 1 --unmapped --ambiguous --non_directional -X 1000 -N 1 -p 2 --score_min L,-0.6,-0.6 -o ${PBS_O_WORKDIR}/../bismark ${BIS_PATH} -1 ${FASTQR1} -2 ${FASTQR2}
#latest version of bismark
echo ":::Bismark Parameters::: /users/PAS0472/osu6369/tools/bismark_v0.17.0/bismark --bowtie2 --path_to_bowtie ~osu6369/tools/bowtie2-current/ --multicore 4 --unmapped --ambiguous --non_directional -X 1000 -N 1 p 2 --score_min L,-0.6,-0.6 -o ${PBS_O_WORKDIR}/../bismark ${BIS_PATH} -1 ${FASTQR1} -2 ${FASTQR2}"
/users/PAS0472/osu6369/tools/bismark_v0.17.0/bismark --bowtie2 --path_to_bowtie ~osu6369/tools/bowtie2-current/ --multicore 4 --unmapped --ambiguous --non_directional -X 1000 -N 1 -p 2 --score_min L,-0.6,-0.6 -o ${PBS_O_WORKDIR}/../bismark ${BIS_PATH} -1 ${FASTQR1} -2 ${FASTQR2}


printf "Creating methylation calls using \"bismark_methylation_extractor\" function:\n"
#make directory
if [ -d ${PBS_O_WORKDIR}/../methExt ];then
    true
else
    mkdir ${PBS_O_WORKDIR}/../methExt
fi

# Full methylation extraction:
printf "Performing "bismark_methylation_extractor" using PE aligned file: \n\t $(ls ${PBS_O_WORKDIR}/../bismark/${FQNAME}*bismark_bt2_pe.sam)\n"
#old bismark version
#~osu6369/tools/bismark-current/bismark_methylation_extractor -p --bedGraph --counts --genome_folder ${BIS_PATH} --comprehensive -o ${PBS_O_WORKDIR}/../methExt ${PBS_O_WORKDIR}/../bismark/${FQNAME}*bismark_bt2_pe.bam
/users/PAS0472/osu6369/tools/bismark_v0.17.0/bismark_methylation_extractor -p --bedGraph --no_overlap --counts --gzip --genome_folder ${BIS_PATH} --comprehensive -o ${PBS_O_WORKDIR}/../methExt ${PBS_O_WORKDIR}/../bismark/${FQNAME}*bismark_bt2_pe.bam
