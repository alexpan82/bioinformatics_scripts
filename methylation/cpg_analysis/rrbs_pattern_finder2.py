...
def main(args):
	...
	### Make windows using this data (wsize bp windows with at least > 5 CpGs). Then plug each window into stat test
		# Slide from CpG to CpG			
		for i in shared:
			try:
				window = [x for x in shared if i <= x <= i + wsize]
			except:
				window = [x for x in shared if i <= x <= shared[-1]]
			# Window cutoff
			b_window = []
			if len(window) >= 5:
				for pos in window:
...
if __name__ == '__main__':
	parser = argparse.ArgumentParser(description='Finds patterns of methylation differences between 2 CpG files using a sliding window analysis and test of equal proportions')
