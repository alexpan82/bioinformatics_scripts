...
for n in sorted(covcount.keys()):
	...
	# I do this so it is easy to find the cumulative # of CpGs that meet a given ratio cutoff
	# for example: covlist[0] = # of CpGs w/ cov=1
	covlist = []
	for i in range(0,max(covcount[n].keys())):
		covlist.append(covcount[n][i+1])
