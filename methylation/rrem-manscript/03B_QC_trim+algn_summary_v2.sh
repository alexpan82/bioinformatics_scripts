...
#for fname in $(cat fastq/test1.txt);do
fbase=$(dirname ${fname})
printf "\t${fbase}:\n"
printf "${fbase}\n">>name.${date}tmp
...
for read in R1 R2;do
    ...
    grep $"Total reads processed:" ${file} | awk '{print($4)}' >>pr-${read}.${date}tmp
    grep $"Reads with adapters:" ${file} | awk '{print($4)}' >>tr-${read}.${date}tmp
