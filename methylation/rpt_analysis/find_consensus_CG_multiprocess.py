...
def findconsensus(filename):
	...
	# I also count how many CGs were covered, regardless whether locations have consensus CpGs 
	# This is an outpout metric that reports the efficacy of how many CpGs we actually end up saving
	# allcgloc
	...
	# This set will contain the relative pos of shared CpGs
	consensus = reduce(lambda s1, s2: s1 & s2, cgloc.values())
	# This set contains the relative pos of ALL CpGs
	allcgloc = reduce(lambda s1, s2: s1 | s2, cgloc.values())
	...
	adjustposall = defaultdict(lambda: ())
	adjustposcon = defaultdict(lambda: ())
	# Used to be insidde if statement
	#if consensus != 0:
	#
	...
	for keys in strings.keys():
		...
		if keys.endswith('+)'):
			...
			for cgpos in allcgloc:
				...
				adjustposall[':'.join(keys[0:3])] = adjustposall[':'.join(keys[0:3])] + (cgpos - lenleft,)
				if cgpos in consensus:
					adjustposcon[':'.join(keys[0:3])] = adjustposcon[':'.join(keys[0:3])] + (cgpos - lenleft,)
		# Deal with rverse comp
			...
			for cgpos in allcgloc:
				...
				adjustposall[':'.join(keys[0:3])] = adjustposall[':'.join(keys[0:3])] + (length - 2- tmppos,)
				if cgpos in consensus:
					adjustposcon[':'.join(keys[0:3])] = adjustposcon[':'.join(keys[0:3])] + (length - 2- tmppos,)
	...
	return (consensus, adjustposcon, adjustposall,)
...
# Splits dictionary into smaller ones
...
################################################################################################################################################***
	...
	# Readic is a dictionary with same struc as orig_reads
	# seq_readic is the same as seq_orig_reads
	# The process index (if this is the nth process, numprocess = n)
		# Main purpose is to name temporary files differently
def findcpgs(datalist):
	readic =  datalist[0]
	seq_readic = datalist[1]
	numprocess = datalist[2]
	bam = datalist[3]
	fasta = datalist[4]

	tmpfasta = bam + ".MULTItmp" + str(numprocess) + ".fasta"
	tmpmuscle = bam + ".MULTItmp"+ str(numprocess) + ".musc"
	tmpoutput = bam + ".ConsensusCpG.bed.MULTItmp" + str(numprocess)
	tmpallcpg = bam + ".MULTItmp.allcpg" + str(numprocess)
	outallcpg = open(tmpallcpg, 'w')

	# Loop thru each unique read name and turn into tmporary fasta, and run muscle many times
	# Open output file

	with open(tmpoutput, 'w') as o:
		for name in readic.keys():
			bedstring = """"""
			count = 0
			# Print bed locations to string (bedstring)
			for bed in readic[name]:
			...
			# Here we count all CpGs, regardless of 'consensus' or not
			adjustposlocALL = returntuple[2]

			for bed in readic[name]:
				if bed[0].startswith('chr'):
					#bed = bed.strip().split('\t')
					bedpos = ':'.join(bed[0:3])
					for keys in adjustposlocALL.keys():
						if bedpos == keys:
							for index in adjustposlocALL[keys]:
								index = int(index)
								start = int(bed[1]) + index
								stop = start + 1
								outallcpg.write("%s\t%s\n" % (bed[0],stop))
							break
					# Output consensus CpGs as bed file and also output methylation info
					...
					for keys in adjustposloc.keys():
						if bedpos == keys:
							for index in adjustposloc[keys]:
								...
								cigartuple = seq_readic[seq_index][1]
								if len(cigartuple) > 1:
								...
								# Call methylation
								meth = callmeth(seq_readic[seq_index][0], index, itr)
...
def main(args):
	...
	### Spawn multiple subprocesses and find consensus CpGs
	print "Done processing. Now running MUSCLE\n"
	# First split up dictionary into multiple smaller dictionaries and put them into a list
	# The list will also contain other info that the function findcpgs needs
	processlist = []
	lengthdic = float(len(orig_reads.keys()))

	for n, item in enumerate(chunks(orig_reads, int(round(lengthdic/multip,0)))):
		processlist.append([item, seq_orig_reads, n, bam, fasta])

	# Pool multiprocessing
	pool = mp.Pool(processes=int(n+1))
	pool.map(findcpgs, processlist)

	# Combine all temporary files generated from subprocesses
	filenames1 = []
	filenames2 = []

	for fnum in range(0,n+1):
		filenames1.append(bam + ".ConsensusCpG.bed.MULTItmp" + str(fnum))
		filenames2.append(bam + ".MULTItmp.allcpg" + str(fnum))

	
	ftmpoutname = bam + ".ConsensusCpG.bed.MULTItmp.TOTAL"
	tmpoutallcpg = bam + ".MULTItmp.allcpg.TOTAL"

	with open(ftmpoutname, 'w') as o:
	...
	output = bam + ".ConsensusCpG.MULTI.bed"
	cpgdic = defaultdict(lambda: [0,0])

	outfile = open(output, 'w')
	outfile.write("#chr\tstart\tstop\tcov\tnumCs\t5mCratio\n")
	with open(ftmpoutname, 'r') as f:
		for line in f:
			line = line.split('\t')
			try:
				# Skip lines with mutations
				if int(line[4]) == 1 or int(line[4]) == -1:
	...
	#############################################################################
	# Now count unique CpGs covered by ambig reads (regardless of consensus CpG)
	totalcset = set()
	with open(tmpoutallcpg, 'r') as f:
		for line in f:
			line = line.strip()
			totalcset.add(line)
	...
	print "Total unique CpGs covered by ambig reads (regardless of consensus CpG):\t%s" % len(totalcset)
################################################################################################################################################***
...
if __name__ == '__main__':
	...
	parser.add_argument('-m', dest='multip', default = 1, type=float, help='Optional: Give number of subprocesses to spawn')
